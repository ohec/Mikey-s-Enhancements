    ...
    */
    init : function(){
        ...
        if(!BBLog.storage("updatepopup."+BBLog.version)){
            ...
            BBLog.popup('news', BBLog.t("update.title").replace(/\{version\}/ig, BBLog.version), '<div style="padding:10px; padding-top:0px; font-size:12px; text-align:center;">'+BBLog.t("update.text.1").replace(/\{hp\}/ig, '<a href="http://getbblog.com" target="_blank">getbblog.com</a>')+'<br/><br/><iframe width="640" height="360" src="//www.youtube.com/embed/WcILyqZ3Ixo?list=PL8XNf8lax18nyZhXxg39oKsRos9lybLdZ&hd=1" frameborder="0" allowfullscreen></iframe></div>');
        }
        ...
        switch(action){
                    ...
                    cont.append($('<div class="importexport">')
                        .append(
                            ...
                            $('<span class="bblog-button export tiny">'+BBLog.t("export.settings")+'</span>').attr("data-help", BBLog.t("export.settings.tooltip")).on("click", function(){
                                $.post(BBLog.serviceUrl+"/en/api/export.settings", {data : JSON.stringify(BBLog._storage)}, function(file){
            ...
            case "advanced.option.themes":
                    ...
                    BBLog.initRequest(function(d){
                        ...
                        var addTheme = function(themeData){
                            if(themeData.info.game.toLowerCase() == BBLog.cache("mode")){
                                ...
                                info += '</div>';
                                container.append('<div data-id="'+themeData.id+'" class="plugin"><div class="title"><span class="bblog-checkbox"></span> <a href="'+themeData.url+'" target="_blank">'+themeData.name+'</a> | '+BBLog.t("theme.creator")+" "+themeData.info.creator+'</div><div class="desc">'+themeData.desc.replace(/[<|>]/ig, "")+'</div><div class="info">'+info+'</div><div class="img"><img src="'+themeData.imgurl+'" alt="" width="100%"/></div></div>');
                            }
                        };
                        if(d["themes-new"]){
                            for(var i in d["themes-new"]) if(typeof storage[d["themes-new"][i].id] != "undefined" && storage[d["themes-new"][i].id]) addTheme(d["themes-new"][i]);
                            for(var i in d["themes-new"]) if(typeof storage[d["themes-new"][i].id] == "undefined" || !storage[d["themes-new"][i].id]) addTheme(d["themes-new"][i]);
                        }
                        container.find(".bblog-checkbox").attr("data-tooltip", BBLog.t("plugingallery.activate")).on("click", function(ev){
                            $(this).toggleClass("active");
                            storage = {};
                            if($(this).hasClass("active")) storage[$(this).closest(".plugin").attr("data-id")] = true;
                            container.find(".plugin").find(".bblog-checkbox").not(this).removeClass("active");
                            BBLog.storage(storageKey, storage);
                            base.showReceipt(BBLog.t("reload.required")+" (F5)");
                            ev.stopPropagation();
                        });
                        for(var i in storage){
                            if(storage[i]) container.find(".plugin[data-id='"+i+"']").find(".bblog-checkbox").addClass("active");
                        }
                ...
                (function(){
                    ...
                    data.container.append($('<div style="margin-top:5px; margin-bottom:5px">').html('<a href="http://getbblog.com/themes/example.css" target="_blank">'+BBLog.t("theme.editor.link1")+'</a> - <a href="http://getbblog.com/board/forum/1349/1/Custom-Themes" target="_blank">'+BBLog.t("theme.editor.link2")+'</a> - <a href="https://www.youtube.com/watch?v=Ypawi2sX9rU" target="_blank">&#10148;  '+BBLog.t("theme.editor.link3")+'</a>'));
                    data.type = "theme";
            ...
            case "advanced.option.plugins":
                ...
                data.container.append($('<div style="margin-top:5px;">').html('<a href="http://getbblog.com/plugins/example.js" target="_blank">'+BBLog.t("plugin.editor.link1")+'</a> - <a href="http://getbblog.com/board/forum/1353/1/Custom-Plugins" target="_blank">'+BBLog.t("plugin.editor.link2")+'</a> - <a href="https://www.youtube.com/watch?v=ce7FTOJSK4g" target="_blank">&#10148;  '+BBLog.t("plugin.editor.link3")+'</a>'));
            ...
            case "bf4.stats.higherlevel":
                ...
                $("#overview-rank .rank").append('<br/><div class="bblog-rank">(BBLog Rank '+rank+')</div>').attr("data-tooltip", BBLog.t("bblog.rank.info"));
            break;
            ...
            case "bblog.mark.team":
                ...
                if(window.location.href.match(/\/soldier\/.*?\//)){
                    var username = $.trim($(".profile-venicestats-header-soldier-info-name span, .soldier-info-name span:last").text());
                ...
                if(window.location.href.match(/\/user\//)){
                    var appendFunc = function(ev){
                        ...
                        if(found !== null){
                            ...
                            storageInner.push({"id" : id, "name" : $(this).attr("data-username"), "source" : BBLog.cache("mode")});
                        }
            ...
            case "general.emblems":
                ...
                var loadGallery = function(data){
                    ...
                    $("#bblog-emblem-gallery").append($('<span class="bblog-button margin-right">'+BBLog.t("general.emblems.upload")+'</span>').on("click", function(){
                        if(BBLog.count(emblem.emblem.data) && BBLog.count(emblem.emblem.data.objects) >= 2){
                            ...
                            html.find(".bblog-button").on("click", function(ev){
                                ...
                                var assetsPaths = {};
                                for(var i in emblemData["objects"]){
                                    var tmp = emblemData["objects"][i];
                                    assetsPaths[tmp.asset] = $("img[data-asset='"+tmp.asset+"']").attr("src");
                                }
                    ...
                    $("#bblog-emblem-gallery").append($('<span class="bblog-button margin-right">'+BBLog.t("general.emblems.powered")+'</span>').on("click", function(){
                        window.open('http://getbblog.com/donate');
                    }));
                    ...
                    inp.one("focus", function(){this.value = ''});
                    ...
                    var sorts = ["install", "time", "random"];
                    var boxes = $('<div class="sorts">');
                    var sort = BBLog.cache("general.emblems.sort") || "install";
                    $.each(sorts, function(key, value){
                        boxes.append('<span class="sort"><span class="bblog-checkbox '+(value == sort ? 'active' : '')+'" data-sort="'+value+'"></span> '+BBLog.t("general.emblems.sort."+value)+'</span>');
                    });
                    boxes.find(".bblog-checkbox").on("click", function(){
                        resetCache();
            ...
            case "bf4.awards.improvements":
                var u = "";
                switch(action){
                    case "bf4.unlocks.improvements": u = "vehicleunlocks|weaponunlocks|kits|dogtags"; break;
                    case "bf4.assignments.improvements": u = "assignments"; break;
                    case "bf4.awards.improvements": u = "awards"; break;
                }
            ...
            case "bf4.vehicle.list.filters":
                ...
                if(action == "bf4.weapons.list.filters"){
                    var buttons = $('<div class="buttons">');
                    buttons.append($('<div class="button all" data-id="all"><span>ALL</span></div>'));
...
BBLog.initIV = setInterval(function(){
    if(typeof BBLogTranslations != "undefined" && typeof S != "undefined" && typeof S.globalContext != "undefined" && typeof Surface != "undefined" && $("body").hasClass("is-loggedin")){
        BBLog._storage = JSON.parse(unescape(BBLogStorageInit));
